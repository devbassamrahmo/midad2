...
const Page = () => {
  ...
  return (
    <div className="px-4 sm:px-6 md:px-10 lg:px-[118px]">
      {/* Header / Intro */}
      <div className="mt-24 sm:mt-32 md:mt-40 lg:mt-[265px] flex flex-col items-center gap-6 sm:gap-8 md:gap-[60px] text-center">
        <h1 className="text-[#4C231A] font-editors font-semibold text-[28px] sm:text-[32px] md:text-[34px] lg:text-[37px]">
        ...
        <p className="text-[#A19F96] font-helvetica text-[15px] sm:text-[16px] md:text-[18px] max-w-[660px]">
          Discover our signature developments that redefine luxury living on the Jeddah Corniche.
          Each project is thoughtfully designed to combine timeless elegance, modern innovation, and
          world-class services — creating spaces that inspire, connect, and elevate everyday life.
        </p>
      </div>
      ...
      <div className="flex flex-col items-center gap-3 sm:gap-4 md:gap-5">
        ...
        <h2 className="text-[#4C231A] font-editors text-center font-semibold text-[18px] sm:text-[20px] md:text-[22px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10 py-12 sm:py-16 md:py-20">
        {data.map((item, i) => (
          <div key={i} className="flex flex-col items-center gap-3 sm:gap-4">
            ...
            <h3 className="text-[#4C231A] font-editors font-semibold text-[18px] sm:text-[20px] md:text-[22px] text-center">
              {item.title}
            </h3>
            <Link
              href={item.link}
